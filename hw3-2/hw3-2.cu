...
__global__ void block_FW_Phase2(int* dist_d, int n, int round){
    ...
    // block with blockIdx.y == 0 handles pivot row
    if(blockIdx.y == 0){ 
        ori_i = round * B + threadIdx.y;
        ori_j = blockIdx.x * B + threadIdx.x + ((blockIdx.x >= round) * B);
    }
    // block with blockIdx.y == 1 handles pivot column
    else{ 
    ...
    for (int k = 0; k < B; k++) {
        int temp;
        temp = s_dist[!blockIdx.y][i][k] + s_dist[blockIdx.y][k][j];
        if(temp < s_dist[0][i][j]){
            s_dist[0][i][j] = temp;
        }
        temp = s_dist[!blockIdx.y][i][k] + s_dist[blockIdx.y][k][j + half_B];
        if(temp < s_dist[0][i][j + half_B]){
            s_dist[0][i][j + half_B] = temp;
        }
        temp = s_dist[!blockIdx.y][i + half_B][k] + s_dist[blockIdx.y][k][j];
        if(temp < s_dist[0][i + half_B][j]){
            s_dist[0][i + half_B][j] = temp;
        }
        temp = s_dist[!blockIdx.y][i + half_B][k] + s_dist[blockIdx.y][k][j + half_B];
        if(temp < s_dist[0][i + half_B][j + half_B]){
            s_dist[0][i + half_B][j + half_B] = temp;
        }
        // __syncthreads();
    }
...
__global__ void block_FW_Phase3(int* dist_d, int n, int round){
    ...
    #pragma unroll 64
    for (int k = 0; k < B; k++) {
        if(s_dist[1][i][k] + s_dist[0][k][j] < self_dis1){
            self_dis1 = s_dist[1][i][k] + s_dist[0][k][j];
        }
        if(s_dist[1][i][k] + s_dist[0][k][j + half_B] < self_dis2){
            self_dis2 = s_dist[1][i][k] + s_dist[0][k][j + half_B];
        }
        if(s_dist[1][i + half_B][k] + s_dist[0][k][j] < self_dis3){
            self_dis3 = s_dist[1][i + half_B][k] + s_dist[0][k][j];
        }
        if(s_dist[1][i + half_B][k] + s_dist[0][k][j + half_B] < self_dis4){
            self_dis4 = s_dist[1][i + half_B][k] + s_dist[0][k][j + half_B];
        }
        // __syncthreads();
    }
